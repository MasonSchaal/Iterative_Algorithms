...
public class IterativeSearcherTest extends TestCase {
	...
	//number of elements in test array
	final int N = 10000;
	//log (base 2) of N represents approximately how many steps should be necessary to do a binary search for a given (unique) value;	
	final int LOG_BASE_2_OF_FILE_SIZE = (int) (Math.log(N) / Math.log(2)) + 1;

	IntegerArray arrayToSearch =  buildArrayToSearch();
	...
	private IntegerArray buildArrayToSearch() {
		...
		IntegerArray integerArray = new IntegerArray(array.length);
		
		for (int i = 0; i < array.length; i++) {
			integerArray.write(i, array[i]);
		}
